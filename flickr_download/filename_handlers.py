...
def title(photo, suffix):
    ...
    """
    if not photo.title:
        return idd(photo, suffix)

    return '{0}{1}.jpg'.format(photo.title, suffix)


def idd(photo, suffix):
    """
    Name file after id

    @param set: Flickr.Photoset, the photoset
    @param photo: Flickr.Photo, the photo
    @param suffice: str, optional suffix
    @return: str, the filename
    """
...
def title_increment(photo, suffix):
    ...
    extra = ''
    photo_index = INCREMENT_INDEX[photo.title]
    if photo_index:
        extra = '({0})'.format(photo_index)
    INCREMENT_INDEX[photo.title] += 1
